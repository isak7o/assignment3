...
app.config['SESSION_COOKIE_SAMESITE'] = 'Lax'  # Protect against CSRF (use 'Strict' if no cross-site requests needed)
app.config['PERMANENT_SESSION_LIFETIME'] = timedelta(minutes=30)  # Limit cookie lifespan to 30 minutes

# Enable CSRF protection for forms
#csrf = CSRFProtect(app)

# Disable autoescaping for all templates (temporary for testing)
#app.jinja_env.autoescape = False

#Define the custom sanitize filter
#@app.template_filter('sanitize')
#def sanitize_filter(text):
#    allowed_tags = ['b', 'i', 'u', 'em', 'strong', 'p', 'ul', 'li', 'br']  # Customize allowed tags
#    allowed_attributes = {}  # Specify allowed attributes if needed
...
def new_post():
    if "current_user" in session.keys():
        ...
        return render_template("no_user.html"), 413
...
def FUN_root():
    # Retrieve all posts
    posts = read_note_from_db(session.get('current_user', 'public'))  # Adjust based on how your function works
